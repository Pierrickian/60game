...
function JokerChoiceOverlay({ options, onChoose, onAdd, canAdd }) {
  ...
  return <div className="joker-choice-overlay">{options.map((option, index) => <motion.button key={option.id} className="joker-choice-btn" initial={{ opacity: 0, y: 12 }} animate={{ opacity: [0.95, 0.9, 0.82], y: [0, -6, 0] }} exit={{ opacity: 0, y: -18, scale: 0.85 }} transition={{ duration: JOKER_CHOICE_DURATION_MS / 1000, ease: 'linear', repeat: Infinity }} onClick={() => onChoose(option.id)}><b>{option.name}</b><small>{option.popupText}</small>{index === options.length - 1 && canAdd ? <span className="joker-choice-plus" onClick={(event) => { event.stopPropagation(); onAdd() }}>+ {RUBY_ICON}</span> : null}</motion.button>)}</div>
}
